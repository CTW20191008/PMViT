...
def inference(model, data_loader):
    ...
    for idx, (img, scramble_img, mask, _) in enumerate(data_loader):
        ...
        puzzles_img = model(img, scramble_img, mask)
        ...
        puzzles_img_1 = puzzles_img.cpu().squeeze(0).detach().numpy().transpose((1, 2, 0))
        ...
        ssim = compare_ssim(img_1, puzzles_img_1, channel_axis=2, data_range=img_1.max() - img_1.min())
        ...
        scramble_img = unnormalize(scramble_img.cpu().squeeze(0)).numpy().transpose((1, 2, 0))
        puzzles_img = unnormalize(puzzles_img.cpu().squeeze(0)).detach().numpy().transpose((1, 2, 0))

        ssim_unnorm = compare_ssim(img, puzzles_img, channel_axis=2, data_range=img.max() - img.min())
        ...
        plt.subplot(1,3,2)
        plt.imshow(scramble_img)
        plt.title('scramble image')
        plt.subplot(1,3,3)
        plt.imshow(puzzles_img)
        plt.title('puzzles image')
        plt.show()
